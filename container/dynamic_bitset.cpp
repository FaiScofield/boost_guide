...
void case1()
{
    ...
    dynamic_bitset<> db3(0x16,  // 大小为22的dynamic_bitset
                         BOOST_BINARY(10101));  // 注意这里，构造了一个编译期的二进制数，没有运行时开销
    dynamic_bitset<> db4(string("0100"));       // 临时变量的拷贝构造，效率低
    dynamic_bitset<> db5(db3);

    dynamic_bitset<> db6;
    db6 = db4;

    cout << hex << db5.to_ulong() << endl; // 转为整数
...
void case2()
{
    ...
    {
        dynamic_bitset<> db(5, BOOST_BINARY(01001));
        // append()把整数转换为一个Block再追加到末尾（最高位），这将使其大小增加一个Block长度
        db.append(BOOST_BINARY(101));
        assert(db.size() == sizeof(unsigned long)*8 + 5);
        cout << db << endl; // 000...(共64-3个0)...10101001
    }
}
...
void case4()
{
    ...
    assert(db.test(0) && !db.test(1));  // 检验第n位是否为1
    assert(db.any() && !db.none());     // 存在/不存在1
    assert(db.count() == 2);            // 所有值为1的元素数量
...
void case6()
{
    ...
    cout << (db1 | db2) << endl; // 交集
    cout << (db1 & db2) << endl; // 并集
    cout << (db1 - db2) << endl; // 差集
